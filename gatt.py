...
class Application(dbus.service.Object):
    ...
    def __init__(self, bus):
        ...
        self.add_service(RobotCommandService(bus, 0))
...
class RobotCommandService(Service):
    """
    Service to send commands to robot.

    """
    RC_UUID = '4028c1a3-6c31-4a75-85a8-3d979ab112a8'

    def __init__(self, bus, index):
        Service.__init__(self, bus, index, self.RC_UUID, True)
        self.add_characteristic(RobotCommandChrc(bus, 0, self))
        self.service_code = 0 #idk if I need this
        print("INIT ROBOT COMMAND SERVICE")


class RobotCommandChrc(Characteristic):
    RC_CMD_UUID = '36fe70f0-d8af-4fc9-b318-ce5a58a27df1'
    ...
    def __init__(self, bus, index, service):
        ...
        print("INIT ROBOT COMMAND CHARACTERISTIC")

        self.channel = grpc.insecure_channel('localhost:50051')
        self.stub = rover_pb2_grpc.ExchangeStub(self.channel)
        print(dir(self.stub))
        # rpm = rover_pb2.RPM(
        #     fr = 0,
        #     fl = 0,
        ...
        #     bl = 0
        #   )
        #status = self.stub.SendControl(rover_pb2.Control(rpm_command=rpm))
        #status = self.stub.UnaryCall(rover_pb2.Control(rpm_command=rpm))
        #status = self.stub.SendControl(rover_pb2.Control(rpm_command=rpm))


    def robot_command_cb(self):
        print("GOT A COMMAND CALLBACK. Printing dir(self)")
        print(dir(self))
        # value = []
    ...
    def WriteValue(self, value, options):
        ...
        if len(value) > 2:
            ...
            try:
                ...
                #print("RPM: %d, Steering %d" %(rpm, steer))
                fr_rpm = rpm - (rpm * steer) / 100.0
                fl_rpm = rpm + (rpm * steer) / 100.0
                br_rpm = fr_rpm
                bl_rpm = fl_rpm
                fl_rpm *= -1
                br_rpm *= -1
...
def main():
    ...
    app = Application(bus) #This is our application, defined in this file.
